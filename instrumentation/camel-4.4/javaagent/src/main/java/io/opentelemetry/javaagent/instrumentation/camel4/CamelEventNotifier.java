...
import static io.opentelemetry.javaagent.instrumentation.camel4.CamelSingletons.getSpanDecorator;
import static io.opentelemetry.javaagent.instrumentation.camel4.CamelSingletons.instrumenter;
...
final class CamelEventNotifier extends EventNotifierSupport {
  ...
  public void notify(CamelEvent event) {
    if (event instanceof CamelEvent.ExchangeSendingEvent exchangeSendingEvent) {
      onExchangeSending(exchangeSendingEvent);
  ...
  private static void onExchangeSending(CamelEvent.ExchangeSendingEvent ese) {
  ...
  /** Camel finished sending (outbound). Finish the span and remove it from the Camel holder. */
  private static void onExchangeSent(CamelEvent.ExchangeSentEvent event) {
